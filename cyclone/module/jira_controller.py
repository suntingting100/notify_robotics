...
class JiraController:
    ...
    def create_issue_by_project_key(self, project, user_id, summary, description):
        issue_dict = {
            'project': {'key': project},
            'summary': summary,
            'priority': {'name': 'P0'},
    ...
    def create_issue_by_business_line(self, line, user_id, summary, description):
        with open(self.config_path) as f:
            json_body = json.load(f)
            project = json_body[line]['project']
            summary = '[%s] %s' % (line, summary)
        issue = self.create_issue_by_project_key(project, user_id, summary, description)
        return issue
